...
void updatehistos(double examplebuffer[MAXFEATURES],
             string  class_label,
             int numberoffields,
...
for (CN=0; CN<numberofclassifiers; CN++)
 {
  numberselected = 0;
  cout<<"\nFor classifier "<<CN<<" and for each field number listed, indicate with 'y' or 'n'\n"<<
...
cout<<"\npredicted_label = "<<predicted_label<<", class_label = "<<class_label;
updatecmatrix(predicted_label, class_label, 
              numberoftypes, typelist, confmatrix);
      if (predicted_label == class_label)
       {
        correct++;
cout<<"\ncorrect";
      ...
      else
       {
        errors++;
        right = false;
        misclassrecords<<auxtestcase->record_number<<","<<class_label<<","<<predicted_label<<endl;
       }
  found = false;
...
for (i=0; i<numfiles; i++)
 {
  filestatlist[i].percentmisclass = 100.0*filestatlist[i].percentmisclass/
    filestatlist[i].totalsamples;
 }
int maxindex;
filestats tempstats;
